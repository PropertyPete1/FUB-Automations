...
async function startServer() {
  ...
  // Lifestyle Bot — 8th virtual agent, auto-texts pond leads 20+ days stale
  ...
  // Sends a warm summary email to Peter & Steven: texts sent, emails sent, any issues
  app.post("/api/scheduled/bot-clockoff", async (req, res) => {
    ...
    try {
      ...
      console.log(`[bot-clockoff] ${result.sent ? "✓ Sent" : "✗ Failed"} — ${result.textsToday} texts, ${result.emailsToday} emails today`);
      res.json({ ok: true, ...result });
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
  ...
  // Uses the Python Phase 2 automation: 14-day cadence, 100 emails/run, live FUB notes, city-tailored
  app.post("/api/scheduled/pond-nurture", async (req, res) => {
    ...
    const cmd = `cd /home/ubuntu/fub_automation && python3 ${scriptPath}`;
    console.log("[pond-nurture] Starting pond nurture run...");
    // Timeout: 35 minutes (2,100,000ms). The script processes up to 100 AI-personalized emails
    // and typically takes 20-30 minutes. 110s was too short and caused false error observations.
    exec(cmd, { timeout: 2100000 }, (err, stdout, stderr) => {
      if (err) {
        const msg = err.message || "script error";
        console.error("[pond-nurture] Script error:", msg);
        console.error("[pond-nurture] stderr:", stderr?.slice(0, 500));
        writeObservation({
          source: "pond_nurture",
          severity: "error",
          category: "script_error",
          message: "Pond nurture script failed",
          detail: msg.slice(0, 500),
          autoFixable: 0,
        }).catch(() => {});
        // Return 200 so heartbeat doesn't retry on script-level errors (idempotency guard inside script)
        res.json({ ok: true, skipped: "script-error", error: msg });
        return;
      }
      const summary = stdout?.slice(-800) || "(no output)";
      console.log("[pond-nurture] Complete:", summary);
      // Parse stdout for sent/skipped/error counts
      const sentMatch = summary.match(/(\d+)\s+email[s]?\s+sent/i);
      const skipMatch = summary.match(/(\d+)\s+skip/i);
      const errMatch  = summary.match(/(\d+)\s+error/i);
      const sentCount = sentMatch ? parseInt(sentMatch[1]) : null;
      const severity  = errMatch ? "warning" : "info";
      writeObservation({
        source: "pond_nurture",
        severity,
        category: "daily_run",
        message: sentCount !== null
          ? `Pond nurture: ${sentCount} emails sent${errMatch ? `, ${errMatch[1]} errors` : ""}`
          : "Pond nurture run completed",
