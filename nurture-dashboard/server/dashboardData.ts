...
try:
    ...
    sms_today = con.execute("SELECT COUNT(*) FROM audit_log WHERE action='pond_nurture' AND status='sms_sent' AND date(created_at)=?", ('${today}',)).fetchone()[0]
    print(json.dumps({'email_today': email_today, 'sms_today': sms_today}))
    con.close()
except Exception as e:
    print(json.dumps({'email_today': 0, 'sms_today': 0}))
`;
    let pond_nurture_today = 0;
    let pond_nurture_sms_today = 0;
    try {
      ...
      pond_nurture_sms_today = todayRaw.sms_today ?? 0;
  ...
  try {
    ...
    if (liveStats) {
      jsonData.live_stats = {
        ...
        pond_nurture_sms_today: liveStats.pond_nurture_sms_today,
...
export async function getPendingQueue(fubApiKey: string, agentFilter?: string): Promise<PendingQueueItem[]> {
  ...
  // FUB sorts by lastActivityAt desc, so the 1-20 day window leads appear
  // contiguously near the top of the list. We paginate until we've seen a
  // full page where ALL leads are older than 20 days — at that point every
  // remaining page is also too old and we can stop early.
  // FUB hard-caps offset at 1900 (returns 400 beyond that), so we never
  // exceed that limit. In practice most agents finish in 1-3 pages.
  const PAGE_SIZE = 100;
    ...
    while (true) {
      ...
      try {
        const data = await fubGet(
          `/people?limit=${PAGE_SIZE}&offset=${offset}&sort=-lastActivityAt&assignedUserId=${uid}`,
          fubApiKey
        );
        page = data.people || [];
      } catch (e) {
      ...
      // Early-exit: if every lead on this page is older than 20 days,
      // all subsequent pages will also be too old (sorted desc by lastActivityAt).
      const allTooOld = page.every(p => {
        const dateStr: string | null = p.lastActivity || p.updated || null;
        if (!dateStr) return false; // keep if no date
        try {
          const daysAgo = Math.floor((Date.now() - new Date(dateStr).getTime()) / MS_PER_DAY);
          return daysAgo > 20;
        } catch { return false; }
      });
      ...
      if (allTooOld || page.length < PAGE_SIZE || offset >= MAX_OFFSET) break;
      offset += PAGE_SIZE;
    }
    return results;
  }
  ...
  for (const uid of agentUserIds) {
    try {
      const agentLeads = await fetchAllEligibleForAgent(uid);
      console.log(`[getPendingQueue] userId=${uid}: fetched ${agentLeads.length} leads (paginated)`);
      perAgentResults.push(agentLeads);
    } catch (e) {
      console.warn(`[getPendingQueue] Per-agent fetch failed for userId=${uid}:`, e);
  ...
  const eligibleCandidates = candidates.filter(person => {
    ...
    // Include leads that are 1–20 days stale.
  ...
  const enriched = await Promise.allSettled(
    eligibleCandidates.map(async person => {
      ...
      if (lastActivityStr) {
        try {
          const lastActivity = new Date(lastActivityStr);
          daysStale = Math.floor((Date.now() - lastActivity.getTime()) / (1000 * 60 * 60 * 24));
        } catch { /* ignore */ }
      }
...
export async function getAgentLeads(fubApiKey: string, agentFirstName: string): Promise<AgentLead[]> {
  ...
  // Fetch leads assigned to this agent — stale-first strategy:
  // 1. First fetch leads with lastActivity older than 14 days (the do_now / hot tier candidates)
  // 2. Then fill remaining capacity with recent leads (your_leads tier)
  // This ensures agents with large rosters (e.g. Peter with 3000+ leads) still show
  // accurate do_now and hot_prospect counts instead of 0.
  const allPeople: any[] = [];
  const pageSize = 100;
  const cutoff14 = new Date(Date.now() - 14 * 24 * 60 * 60 * 1000).toISOString().split("T")[0] + "T00:00:00Z";

  // Pass 1: stale leads (14+ days) — up to 200
  let offset = 0;
  while (allPeople.length < 200) {
    const page = await fubGet(
      `/people?limit=${pageSize}&offset=${offset}&assignedUserId=${targetUserId}&lastActivityBefore=${cutoff14}`,
      fubApiKey
    );
  ...
  // Pass 2: recent leads (0-13 days) — fill up to 300 total
  if (allPeople.length < 300) {
    const recentPage = await fubGet(
      `/people?limit=${300 - allPeople.length}&offset=0&assignedUserId=${targetUserId}`,
      fubApiKey
    );
    const recentBatch: any[] = recentPage.people || [];
    // Deduplicate by id
    const seenIds = new Set(allPeople.map((p: any) => p.id));
    for (const p of recentBatch) {
      if (!seenIds.has(p.id)) allPeople.push(p);
    }
  }
  ...
  const enriched = await Promise.allSettled(
    capped.map(async (person): Promise<AgentLead> => {
      ...
      // Classify tier
      // do_now: 14+ days stale (includes 21+ which are at risk of auto-reassignment to pond)
      // hot_prospect: FUB stage = "Hot Prospect" (regardless of staleness)
      // your_leads: everything else (0-13 days stale, non-hot)
