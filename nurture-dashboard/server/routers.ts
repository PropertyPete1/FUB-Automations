...
export const appRouter = router({
  ...
  ai: router({
    ...
    draftSms: publicProcedure
      ...
      .mutation(async ({ input }) => {
        const { leadName, leadCity, daysStale, assignedAgent, notes, prefillMessage } = input;
...
Original: ${prefillMessage || `Hey ${leadName}, hope you had a great week! Still thinking about homes in ${leadCity || "Texas"}? 😊`}`;
    ...
    draftReply: publicProcedure
      ...
      .mutation(async ({ input }) => {
        const { leadName, leadCity, assignedAgent, inboundMessage, notes } = input;

        const systemPrompt = `You are a Texas real estate agent named ${assignedAgent || "the agent"} at Lifestyle Design Realty. Your job is to write a warm, natural SMS reply to a lead's message.
...
Write a natural reply that addresses their message and keeps the conversation going. Under 160 chars.`;
  ...
  bot: router({
    /**
     * Returns today's SMS counts for all agents + Lifestyle Bot,
