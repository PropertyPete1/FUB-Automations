 ...
 *      b. If lead has a valid phone → remove bad email, add "bad-email" tag, add FUB note
 *      c. If lead has no phone → move to Trash stage in FUB, add FUB note
...
async function removeBadEmailAndTag(person: FubPerson, badEmail: string): Promise<void> {
  ...
  await fubPost("/notes", {
    ...
    body: `Automated bounce handler removed bad email address "${badEmail}" on ${dateStr}. Gmail returned a permanent delivery failure (inbox full or address invalid). Lead has a valid phone number on file — kept in system with "bad-email" tag. Will continue to receive SMS outreach from Lifestyle Bot.`,
    isHtml: false,
  });
}
...
async function moveLeadToTrash(person: FubPerson, badEmail: string): Promise<void> {
  ...
  await fubPost("/notes", {
    personId: person.id,
    subject: "Lead Moved to Trash — Bounce Handler",
    body: `Automated bounce handler moved this lead to Trash on ${dateStr}. Gmail returned a permanent delivery failure for "${badEmail}" (inbox full or address invalid). No valid phone number on file — no other way to reach this lead. Removed from all automation.`,
    isHtml: false,
  });
}
  ...
  try {
    ...
    for (const email of bouncedEmails) {
      try {
        ...
        if (phone) {
          // Has a valid phone — remove bad email, add tag, keep in system
          await removeBadEmailAndTag(lead, email);
          result.emailRemovedPhoneKept++;
          result.details.push(`KEPT (has phone): ${lead.name} — removed ${email}, added bad-email tag`);
          console.log(`[bounceHandler] Kept ${lead.name} (ID ${lead.id}) — removed bad email, has phone ${phone}`);

          await writeObservation({
            source: "bounce_handler",
            severity: "info",
            category: "email_bounce",
            message: `Bad email removed — ${lead.name} kept (has phone)`,
            detail: `Removed ${email} from FUB. Phone ${phone} is valid. Added bad-email tag.`,
            autoFixable: 0,
            runId,
          });
        } else {
          // No phone — move to trash
