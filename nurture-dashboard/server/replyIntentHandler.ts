...
async function classifyReplyIntent(
  ...
  const systemPrompt = `You are an AI assistant that classifies real estate email replies.
Your job is to determine whether an email reply indicates the sender wants to opt out of real estate communications.

Opt-out signals to detect (any of these = isOptOut: true):
...
NOT opt-out signals (isOptOut: false):
- Questions about properties or listings
- Asking for more information
- Scheduling a showing or call
- Positive engagement or interest
- Out-of-office auto-replies (these are automated, not a human opt-out)
- Vacation responders
- General conversation or chitchat
- Asking about a specific property

Be conservative: only set isOptOut: true when there is clear, unambiguous evidence.
...
Return JSON with: isOptOut (boolean), confidence (0.0-1.0), reason (short explanation under 200 chars).`;
  ...
  try {
      ...
      response_format: {
        ...
        json_schema: {
          ...
          schema: {
            ...
            required: ["isOptOut", "confidence", "reason"],
    ...
    return { isOptOut: false, confidence: 0, reason: "LLM classification failed" };
  }
}
...
async function applyOptOutTag(person: FubPerson, reason: string): Promise<void> {
  // Preserve all existing tags and add opt-out
  const currentTags = person.tags ?? [];
  if (currentTags.some(t => t.toLowerCase() === "opt-out")) {
    // Already has opt-out tag — nothing to do
    return;
  }
  const updatedTags = [...currentTags, "opt-out"];

  await fubPut(`/people/${person.id}`, { tags: updatedTags });

  // Leave a FUB note documenting the auto-detection
  ...
  await fubPost("/notes", {
    personId: person.id,
    subject: "Auto-Detected Opt-Out Reply — Removed from Automation",
    body: `Automated reply intent detector flagged this lead on ${dateStr}.\n\nLead replied to a pond nurture email indicating they are no longer interested or already taken care of.\n\nReason detected: ${reason}\n\nAction taken: Added "opt-out" tag. Lead will no longer receive pond nurture emails, Lifestyle Bot texts, or any automated outreach.`,
    isHtml: false,
  });
}
