...
describe("MonitorResult findings structure", () => {
  ...
  it("includes cloud computer health check", async () => {
    const result = await runBotMonitor("manual");
    // The old 'Critical automation files' check was replaced with 'Cloud computer automation'
    // which correctly handles the production environment where local files are not accessible
    const hasCloudCheck = result.findings.some(f => f.check.toLowerCase().includes("cloud"));
    expect(hasCloudCheck).toBe(true);
  });
});
