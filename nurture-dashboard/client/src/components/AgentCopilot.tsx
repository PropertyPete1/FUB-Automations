    ...
    <>
      ...
      {isOpen && (
        ...
        >
          ...
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-amber-100 bg-gradient-to-r from-stone-950 to-[#0C0C0E] shrink-0">
            ...
            <div className="flex items-center gap-1">
              {messages.length > 0 && !isMinimized && (
                <button
                  onClick={clearChat}
                  className="px-2 py-1 text-[10px] text-slate-400 hover:text-slate-600 transition-colors rounded font-mono uppercase tracking-wider"
                >
                  Clear
                </button>
              )}
          ...
          {!isMinimized && (
            <>
                ...
                <div className="px-3 py-2 border-b border-white/5 shrink-0">
                  <div className="relative">
                    <button
                      ...
                      className="w-full flex items-center justify-between px-3 py-2 rounded-lg bg-slate-50 hover:bg-white/8 border border-white/8 text-xs transition-colors"
                    >
                      ...
                      <ChevronDown className={cn("h-3.5 w-3.5 text-slate-400 transition-transform", showLeadPicker ? "rotate-180" : "")} />
                    </button>

                    {showLeadPicker && (
                      <div className="absolute top-full left-0 right-0 mt-1 bg-[#141416] border border-[#E4E7EF] rounded-xl shadow-2xl z-10 max-h-48 overflow-y-auto">
                        <button
                          onClick={() => { setSelectedLead(null); setShowLeadPicker(false); }}
                          className="w-full text-left px-3 py-2 text-xs text-slate-400 hover:bg-slate-50 transition-colors border-b border-white/5"
                        >
                          No lead context
                        </button>
                        {leads.map((lead) => (
                          <button
                            ...
                            className={cn(
                              "w-full text-left px-3 py-2.5 text-xs transition-colors hover:bg-slate-50",
                              selectedLead?.id === lead.id ? "bg-amber-500/10 text-amber-700" : "text-slate-600"
                            )}
                          >
                            <div className="font-medium">{lead.name}</div>
                            <div className="text-slate-400 mt-0.5">{lead.city || "Texas"} · {lead.days_stale}d stale · {lead.assigned_agent}</div>
                          </button>
                        ))}
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-2 pb-1">
                      {quickActions.map((action) => (
                        <button
                          ...
                          className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-slate-50 hover:bg-white/8 border border-white/8 hover:border-amber-500/30 text-xs text-slate-600 hover:text-white transition-all duration-150 text-left disabled:opacity-50 disabled:cursor-not-allowed group"
                        >
                    ...
                    <div className="flex flex-col space-y-4 p-4">
                        ...
                        >
                          ...
                          <div
                            className={cn(
                              ...
                              message.role === "user"
                                ? "bg-amber-500/15 text-amber-100 border border-amber-200"
                                : "bg-slate-50 text-slate-700 border border-white/8"
                            )}
                          >
                            {message.role === "assistant" ? (
                              <div className="prose prose-sm prose-invert max-w-none [&_p]:text-slate-700 [&_strong]:text-white [&_li]:text-slate-600 [&_code]:text-amber-700 [&_code]:bg-slate-50 [&_code]:px-1 [&_code]:rounded">
                      ...
                      {chatMutation.isPending && (
                        <div className="flex items-start gap-2.5">
                          ...
                          <div className="rounded-xl bg-slate-50 border border-white/8 px-4 py-3 flex items-center gap-2">
                ...
                <div className="px-3 py-2 border-t border-white/5 flex gap-1.5 overflow-x-auto shrink-0 scrollbar-none">
                  {quickActions.map((action) => (
                    <button
                      ...
                      onClick={() => sendMessage(action.prompt)}
                      disabled={chatMutation.isPending}
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-50 hover:bg-white/8 border border-white/8 hover:border-amber-500/30 text-[11px] text-slate-500 hover:text-slate-700 transition-all whitespace-nowrap disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
                    >
              ...
              <div className="px-3 pb-3 pt-2 border-t border-white/5 shrink-0">
                <div className="flex gap-2 items-end bg-slate-50 border border-[#E4E7EF] rounded-xl px-3 py-2 focus-within:border-amber-500/40 transition-colors">
                  <Textarea
                    ref={textareaRef}
                    value={input}
                    ...
                    className="flex-1 max-h-28 resize-none min-h-[36px] bg-transparent border-none shadow-none focus-visible:ring-0 text-sm text-slate-700 placeholder:text-slate-300 p-0"
                    rows={1}
                  />
                  <button
                ...
                <p className="hidden sm:block text-[10px] text-slate-300 mt-1.5 text-center">
